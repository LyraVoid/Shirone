...
func (_q *MediaAssetQuery) QueryOwner() *UserQuery {
...
func (_q *MediaAssetQuery) WithOwner(opts ...func(*UserQuery)) *MediaAssetQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *MediaAssetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*MediaAsset, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, mediaasset.ForeignKeys...)
	}
	...
	if query := _q.withOwner; query != nil {
		if err := _q.loadOwner(ctx, query, nodes, nil,
			func(n *MediaAsset, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MediaAssetQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*MediaAsset, init func(*MediaAsset), assign func(*MediaAsset, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*MediaAsset)
	for i := range nodes {
		if nodes[i].user_media_assets == nil {
			continue
		}
		fk := *nodes[i].user_media_assets
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_media_assets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
