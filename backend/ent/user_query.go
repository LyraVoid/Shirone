...
func (_q *UserQuery) loadDocuments(ctx context.Context, query *DocumentQuery, nodes []*User, init func(*User), assign func(*User, *Document)) error {
	...
	for _, n := range neighbors {
		fk := n.user_documents
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_documents" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_documents" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
