...
func (_q *CommentQuery) QueryDocument() *DocumentQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, comment.DocumentTable, comment.DocumentColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *CommentQuery) WithDocument(opts ...func(*DocumentQuery)) *CommentQuery {
	query := (&DocumentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if _q.withAuthor != nil || _q.withDocument != nil || _q.withParent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withDocument; query != nil {
		if err := _q.loadDocument(ctx, query, nodes, nil,
			func(n *Comment, e *Document) { n.Edges.Document = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *CommentQuery) loadDocument(ctx context.Context, query *DocumentQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Document)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i].document_comments == nil {
			continue
		}
		fk := *nodes[i].document_comments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "document_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
