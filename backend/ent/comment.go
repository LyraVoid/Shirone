...
// was not loaded in eager-loading, or loaded but was not found.
func (e CommentEdges) DocumentOrErr() (*Document, error) {
	if e.Document != nil {
...
func (_m *Comment) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case comment.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field document_comments", value)
			} else if value.Valid {
				_m.document_comments = new(int)
				*_m.document_comments = int(value.Int64)
			}
...
// QueryDocument queries the "document" edge of the Comment entity.
func (_m *Comment) QueryDocument() *DocumentQuery {
	return NewCommentClient(_m.config).QueryDocument(_m)
}
